...
class PageVC: UIPageViewController {
    ...
    func configureListButton(){
        ...
        listButton.addTarget(self, action: #selector(segueToLocationCV), for: .touchUpInside)
        view.addSubview(listButton)
    }
    
    @objc func segueToLocationCV(){
...
extension PageVC: UIPageViewControllerDataSource, UIPageViewControllerDelegate{
    ...
    @objc func pageControlPressed(){
        guard let currentViewControler = self.viewControllers?[0] as? DetailVC else{return}
            currentPage = currentViewControler.currentPage
